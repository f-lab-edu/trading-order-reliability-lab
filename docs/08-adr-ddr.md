...
| `ADR-011` | Reconciliation 실행 상태는 Recovery Service가 소유하고, Order Service는 복구 필요/결과 상태만 소유한다 | Accepted |
...
## ADR-011. Reconciliation 실행 상태는 Recovery Service가 소유하고, Order Service는 복구 필요/결과 상태만 소유한다
...
Reconciliation은 두 가지 성격을 가진다.

첫째, 주문 관점에서는 외부 상태 확인이 필요한지, 복구가 성공했는지, 복구가 실패했는지를 표현해야 한다.

둘째, 복구 workflow 관점에서는 reconciliation job이 대기 중인지, 실행 중인지, 재시도 중인지, 실패했는지를 관리해야 한다.

이 둘을 모두 Order Service의 주문 상태에 넣으면 Order Service가 Recovery Service의 job 실행 상태까지 알아야 한다. 반대로 Recovery Service가 주문 상태를 직접 변경하면 주문 상태 소유권이 분산된다.

따라서 주문의 복구 필요/결과 상태와 reconciliation job의 실행 상태를 분리해야 한다.

### Options

| 대안 | 설명 | 평가 |
|---|---|---|
| Order Service가 reconciliation 실행 상태까지 모두 관리 | 주문에 `PENDING`, `RUNNING`, `RESOLVED`, `FAILED`를 모두 둠 | Recovery job 실행 상태가 Order 도메인에 섞임 |
| Recovery Service가 주문 상태까지 직접 변경 | Recovery가 job 결과에 따라 주문 상태를 직접 갱신 | 주문 상태 소유권이 깨짐 |
| Order Service는 복구 필요/결과 상태만 소유하고, Recovery Service가 job 실행 상태를 소유 | 주문 상태와 복구 workflow 상태를 분리 | 책임 경계가 명확함 |

### Decision

Order Service는 주문의 reconciliation 상태를 다음 값으로만 관리한다.
...
`RUNNING`은 Order Service의 주문 상태에 두지 않는다.

Recovery Service는 reconciliation job의 실행 상태를 별도로 관리한다.
...
상태조회 workflow 실패는 Recovery Service가 판단하고 `ReconciliationJobFailed` 이벤트로 Order Service에 알린다.

Snapshot을 수신했지만 도메인 상태로 수렴시킬 수 없는 실패는 Order Service가 판단하고 `OrderReconciliationFailed` 이벤트로 Recovery Service에 알린다.

### Consequences

장점:

- Order Service가 Recovery job 실행 상태에 종속되지 않는다.
- Recovery Service가 주문 상태를 직접 변경하지 않는다.
- workflow failure와 domain resolution failure를 분리할 수 있다.
- `reconciliationStatus = RUNNING` 동기화를 위한 불필요한 이벤트가 사라진다.

단점:

- Recovery Service에서 Order Service로 전달하는 recovery event topic이 필요하다.
- 실패 이벤트의 방향과 의미를 명확히 문서화해야 한다.
...
`RECONCILING` 또는 `RUNNING`을 주문 상태에 넣으면 주문의 비즈니스 상태와 복구 workflow 실행 상태가 섞인다.

주문 관점에서 필요한 것은 복구가 필요한지, 복구가 성공했는지, 복구가 실패했는지다.  
실제 상태조회 job이 실행 중인지 여부는 Recovery Service가 관리해야 한다.
...
`RUNNING`은 `Order.reconciliationStatus`에 두지 않는다.  
Reconciliation job의 실행 상태는 Recovery Service의 job 상태로 관리한다.

### Consequences

장점:

- 주문의 비즈니스 상태와 복구 결과 상태를 분리할 수 있다.
- Recovery job의 실행 상태가 Order 도메인에 섞이지 않는다.
- `UNKNOWN + PENDING`, `LIVE + PENDING`, `PARTIALLY_FILLED + PENDING` 같은 조합이 가능하다.

단점:

- 실제 reconciliation job이 실행 중인지 확인하려면 Recovery Service의 job 상태를 함께 조회해야 한다.
...
## DDR-012. Reconciliation 실패는 workflow failure와 domain resolution failure로 분리한다
...
두 실패를 같은 이벤트로 처리하면 실패 원인과 책임 주체가 불명확해진다.

### Decision

Reconciliation 실패를 두 종류로 분리한다.
...
다음 상황은 Recovery Service가 판단한다.

- 상태조회 command 재시도 한도 초과
- 상태조회 응답 미수신
- Gateway 상태조회 처리 실패 반복
- Recovery job 자체 실패

Recovery Service는 `ReconciliationJobFailed`를 발행한다.  
Order Service는 이를 수신해 `reconciliationStatus = FAILED`로 반영한다.

#### Domain resolution failure

다음 상황은 Order Service가 판단한다.

- `snapshotStatus = NOT_FOUND`
- snapshot 수량이 주문 수량 불변식을 위반함
- snapshot이 현재 terminal 상태와 충돌함
- snapshot을 적용할 유효한 상태 전이가 없음

Order Service는 `OrderReconciliationFailed`를 발행한다.  
Recovery Service는 이를 수신해 reconciliation job을 `FAILED`로 종료한다.

### Consequences

장점:

- 상태조회 workflow 실패와 도메인 상태 수렴 실패가 구분된다.
- 실패 원인을 운영자가 더 명확히 추적할 수 있다.
- 각 서비스가 자신이 판단 가능한 실패만 판단한다.

단점:

- 실패 이벤트가 두 종류로 늘어난다.
- 이벤트 방향을 혼동하지 않도록 명확한 문서화가 필요하다.
