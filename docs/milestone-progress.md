...
| 최신 전체 검증 | 2026-06-25 `./gradlew --gradle-user-home .gradle test --rerun-tasks` 성공 |
...
* `CXLQ` response timeout 이후 `PENDING_CANCEL -> UNKNOWN`
* cancel outcome unknown 기반 reconciliation과 상태조회 snapshot 수렴
* OUT journal unique 제약과 dispatch token/owner/lock 기반 다중 dispatcher hardening
