...
| --- | --- |
| 마지막 갱신 | 2026-06-16 |
| 현재 active milestone | `M5` 기본 취소 흐름 착수 대기 |
| M2 상태 | 완료된 기반으로 취급 |
| M3 상태 | 완료 |
| M4 상태 | 완료 |
| 다음 권장 작업 | M5 설계문서 확인 후 기본 취소 흐름 계획 수립 |
| 최신 완료 milestone 작업 로그 | `local-notes/ai-work-log/2026-06-13-M4.md` |
| 최신 전체 검증 | 2026-06-16 `./gradlew --gradle-user-home .gradle test` 성공 |
...
| `M5` 기본 취소 흐름 | 착수 대기 | `LIVE`/`PENDING_ACK` 주문 취소 요청, Gateway cancel command, 취소 ACK/Reject 반영 설계 확인 후 착수 |
...
* Gateway는 M4 범위 밖 `CancelOrderCommand`, `QueryOrderStatusCommand`를 TCP 송신하지 않고 `UNSUPPORTED_COMMAND`로 parking한다.
* Gateway command dispatch는 설계된 `transport_state` 값만 사용한다. `CREATED + ack_deadline_at IS NULL`만 dispatch claim 대상이고, claim 시 `ack_deadline_at`을 세팅해 재전송 금지 경계로 삼는다.
* `CREATED/SENT + ack_deadline_at` 만료 attempt는 재전송하지 않고 내부 상태 `UNKNOWN` 및 `SUBMIT_OUTCOME_UNKNOWN` parking으로 격리한다. `UNKNOWN` canonical event 발행은 M7 범위로 남긴다.
