...
| `M5.5` | Dispatch fencing hardening | M6 경합 구현 전에 Gateway command dispatch worker lease와 broker 응답 deadline을 분리한다. |
...
`M5.5`는 M5 완료 후 발견한 dispatch fencing hardening을 M6 전에 특별 편성한 마일스톤이다.
범위는 Broker Gateway `broker_command_attempt`의 `dispatch_token`, `dispatch_owner`, `dispatch_locked_until` 도입, `ack_deadline_at` 의미를 `SENT` 이후 broker 응답 deadline으로 축소, stale worker write 방어, OUT journal 이후 직접 재송신 금지 정책 보존, `broker_message_journal` OUT 전용 unique 제약 추가다.
M5.5는 Broker Gateway 내부 DB schema와 dispatch 설정 변경이며, public HTTP API, Kafka event contract, broker wire protocol은 변경하지 않는다.
M5.5에서 submit/cancel OUT journal 이후 불확실성은 Gateway attempt `UNKNOWN`/parking까지만 다루며, canonical unknown event 발행과 Order Service `UNKNOWN` 상태 수렴은 M7/M8 범위다.
주문 상태 도메인, cancel/fill 수량 경합, cancel `UNKNOWN` 상태 수렴과 reconciliation은 M5.5 범위가 아니다.
...
* Gateway가 식별한 `CXLQ` command timeout의 canonical outcome unknown event 발행
