...
* `UNKNOWN`, stale order, EOD non-terminal 주문의 복구 필요 판단은 **Order Service**가 담당한다.
* 상태조회 기반 reconciliation workflow는 **Recovery Service**가 담당한다.
...
flowchart LR
    ...
    BG -->|표준화된 브로커 이벤트 / 상태조회 attempt report| MB
    MB -->|브로커 이벤트 전달| OS

    MB -->|주문 생명주기 이벤트 / 상태조회 attempt report 전달| RS
...
2. **Broker Gateway는 외부 브로커 통신을 격리한다.**
   ...
   - Gateway는 상태조회 command의 전문 송수신 결과를 Recovery Service가 기록할 수 있도록 attempt report로 전달한다.
   - Order Service는 TCP 전문 포맷과 브로커별 식별자 구조를 알지 않는다.

3. **Recovery Service는 복구 흐름을 오케스트레이션한다.**

   * Order Service가 발행한 reconciliation 요청을 받아 job과 attempt를 관리한다.
   * 상태조회 command 발행, Gateway attempt report 수신, 재시도, workflow 실패 처리를 담당한다.
...
- 주문 상태머신 실행
- 주문 수량 불변식 검증
- Phase 1 시장 상태 `OPEN` / `CLOSED` 보유
- 시장 상태 기반 주문 생성 검증
- 외부 브로커 canonical event 적용
- `UNKNOWN` 상태 반영
- stale non-terminal 주문 탐지
- EOD non-terminal DAY 주문 탐지
- reconciliation 요청 이벤트 발행
- reconciliation 결과 반영
- active `CANCEL` instruction 유지 및 cancel command 재발행 판단
- 주문 이벤트 이력 생성
...
- 정교한 장전/정규장/장후/휴장 캘린더 관리
- 독립 Market Service 역할 수행
- TCP 연결 관리
- 전문 파싱/직렬화
- 브로커별 프로토콜 처리
...
* broker event 발행
* 상태조회 command의 attempt report 발행
* command timeout 감지
...
Recovery Service는 Order Service가 요청한 reconciliation workflow를 오케스트레이션한다.

### 주요 책임

* reconciliation job 생성
* 상태조회 command 생성
* 상태조회 attempt report 수신 및 attempt 이력 갱신
...
* 주문 상태 직접 변경
* Order DB를 직접 조회해 stale/EOD 대상을 탐지
* active `CANCEL` instruction을 해석해 cancel command를 직접 재발행
* TCP 전문 직접 송수신
* 브로커 전문 파싱
* 사용자-facing API 제공
...
## 7.8.5 Status Query Attempt Report 흐름
...
### Status Query Attempt Report 종류

* StatusQueryAttemptReported

이 report는 주문 상태를 변경하기 위한 도메인 이벤트가 아니다.
Recovery Service가 `reconciliation_attempt`에 상태조회 전문 ID, Gateway 처리 결과, snapshot 요약 상태, 오류 정보를 기록하기 위한 workflow 관측 이벤트다.
브로커 snapshot의 도메인 해석과 주문 상태 반영은 `BrokerOrderStatusSnapshot`을 수신한 Order Service가 수행한다.
