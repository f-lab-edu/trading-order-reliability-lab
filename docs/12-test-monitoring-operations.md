...
M7 목표 시나리오다. M5.5 기준으로는 Gateway 내부 attempt `UNKNOWN`/parking과 직접 재송신 금지를 먼저 검증한다.

시나리오:

1. 주문 생성 후 Gateway가 `ORDR` 전문을 송신한다.
2. Broker Simulator가 ACK/Reject를 반환하지 않는다.
3. M5.5 기준 Gateway는 `SENT + ack_deadline_at` 만료 또는 OUT `ORDR` journal 이후 불확실한 `CREATED` attempt를 `UNKNOWN`/parking으로 격리하고 직접 재송신하지 않는다.
4. M7 구현에서 Gateway가 `BrokerCommandOutcomeUnknown(commandType=SUBMIT)`을 발행한다.
5. Order Service가 주문을 `UNKNOWN`으로 전환한다.
6. Order Service가 `OrderReconciliationRequested(triggerType=SUBMIT_OUTCOME_UNKNOWN)` 발행한다.
7. Recovery Service가 reconciliation job을 생성한다.
8. 상태조회 snapshot에 따라 주문이 수렴한다.

성공 기준:

* 주문은 `REJECTED`로 임의 종결되지 않는다.
* M5.5 기준 `broker_command_attempt.transport_state`는 `UNKNOWN`으로 기록되고 관련 parking이 남는다.
* M7 기준 `reconciliationStatus = PENDING`으로 전환된다.

---

## 12.7.8 cancel 결과 불확실

M8 목표 시나리오다. M5.5 기준으로는 Gateway 내부 attempt `UNKNOWN`/parking과 직접 재송신 금지를 먼저 검증한다.
...
4. M5.5 기준 Gateway는 `SENT + ack_deadline_at` 만료 또는 OUT `CXLQ` journal 이후 불확실한 `CREATED` attempt를 `UNKNOWN`/parking으로 격리하고 직접 재송신하지 않는다.
5. M8 구현에서 Gateway가 `BrokerCommandOutcomeUnknown(commandType=CANCEL)`을 발행한다.
6. Order Service가 주문을 `UNKNOWN`으로 전환하고 active `CANCEL` instruction을 유지한다.
7. Recovery Service가 상태조회 job을 수행한다.
8. snapshot이 여전히 활성 상태이면 Order Service가 cancel command 재발행 여부를 결정한다.

성공 기준:

* timeout만으로 같은 cancel command를 Gateway가 직접 재전송하지 않는다.
* M5.5 기준 `broker_command_attempt.transport_state`는 `UNKNOWN`으로 기록되고 `CANCEL_OUTCOME_UNKNOWN` parking이 남는다.
...
| --- | ---: | --- |
| `SUBMIT` | 5초 | M5.5: Gateway attempt `UNKNOWN`/parking, M7: `BrokerCommandOutcomeUnknown(commandType=SUBMIT, unknownReason=TIMEOUT)` |
| `CANCEL` | 5초 | M5.5: Gateway attempt `UNKNOWN`/parking, M8: `BrokerCommandOutcomeUnknown(commandType=CANCEL, unknownReason=TIMEOUT)` |
...
5. Gateway의 `broker_command_attempt`를 `orderId` 기준으로 조회하고 `transport_state`, `dispatch_owner`, `dispatch_locked_until`, `ack_deadline_at`을 확인
6. `dispatch_token`이 남아 있으면 lock 만료 여부와 stale owner 가능성을 확인
7. `wireMessageId`로 `broker_message_journal` 송수신 전문 확인
8. OUT journal은 있으나 `SENT`가 아닌 attempt는 외부 도달 여부가 불확실한 것으로 분류
9. `reconciliation_job`과 `reconciliation_attempt` 확인
10. 관련 outbox message가 `FAILED` 또는 오래된 `READY`인지 확인
11. 관련 consumer parking 또는 error log 확인
12. 최종적으로 workflow failure인지 domain resolution failure인지 분류
...
* Gateway command dispatch는 `dispatch_token`과 `dispatch_locked_until`으로 worker lease를 방어한다.
* `ack_deadline_at`은 `SENT` 이후 broker response deadline으로만 사용한다.
* submit/cancel timeout은 직접 재전송하지 않는다. M5.5에서는 Gateway attempt `UNKNOWN`/parking으로 격리하고, Order Service `UNKNOWN` 전환은 M7/M8에서 다룬다.
